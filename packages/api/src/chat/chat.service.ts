...
export class ChatService {
  ...
  async chat(dto: ChatRequest): Promise<ChatResponse> {
    const systemPrompt = this.buildSystemPrompt(dto.context, dto.lang, dto.candidateName);
  ...
  private buildSystemPrompt(context: string, lang: string, candidateName: string): string {
...
${context}`;
  ...
  private sanitizeReply(content: string, lang: string): string {
    ...
    if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
      try {
        JSON.parse(trimmed);
        this.logger.warn('Model returned raw JSON as reply — likely unsupported tool call format');
    ...
    const withoutInlineJson = trimmed.replaceAll(/\{["']?name["']?\s*:\s*["']record_\w+["'][^}]*\}/g, '').trim();
    if (withoutInlineJson !== trimmed) {
      this.logger.warn('Model embedded tool call JSON in text response — stripped');
  ...
  private handleToolCall(name: string, args: Record<string, unknown>): string {
    if (name === 'record_user_details') {
      this.logger.log(`[Tool] record_user_details: email=${String(args.email)}, name=${String(args.name ?? 'N/A')}`);
    } else if (name === 'record_unknown_question') {
      this.logger.log(`[Tool] record_unknown_question: ${String(args.question)}`);
    }
